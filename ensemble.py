...
def train(data_dir, model_dir, args):
    ...
    # -- loss & metric
    criterion_mask = create_criterion('label_smoothing')  # default: cross_entropy
    criterion_age_gender = create_criterion(args.criterion)
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                print("mask_acc_item:", mask_acc_item, mask_labels.shape)
                print("age_gender_acc_item:", age_gender_acc_item)
                ...
                for idx in range(args.valid_batch_size):
                    if mask_labels[idx] == mask_preds[idx] and age_gender_labels[idx] == age_gender_preds[idx]:
                        # acc_item = acc_item + mask_labels[idx] + age_gender_labels[idx]
                        acc_item += 1
                # acc_item = ((mask_labels == mask_preds) * (age_gender_labels == age_gender_preds)).sum().item()
